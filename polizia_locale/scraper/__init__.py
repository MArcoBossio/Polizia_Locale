...
def _phase_timeout(timeout: int, heavy: bool = False) -> tuple[int, int]:
    """Timeout più aggressivo per i primi livelli, un po' più largo sul heavy path."""

    if heavy:
        return (min(timeout, 2), min(timeout, 4))
    return (min(timeout, 3), min(timeout, 5))
...
def _get_cached_page(
    ...
    """Fetch singolo con cache per-run per evitare richieste duplicate."""
...
def _candidate_links(html: str, base: str, page_is_pl: bool = False) -> list[tuple[str, int]]:
...
def _broad_candidate_links(html: str, base: str, limit: int = 8, page_is_pl: bool = False) -> list[str]:
...
def _should_try_browser_fallback(html: str, page_url: str, site_root: str = "", *, url_is_pl: bool = False) -> bool:
    """Decide se vale la pena invocare Playwright/OCR su questa pagina.

    La regola pratica è:
    - sempre sì sulle pagine già riconosciute come PL;
    - sì sulle pagine molto povere di contenuto e senza link crawlabili;
    - no sulle homepage generiche che hanno già candidate HTTP da seguire.
    """

    if url_is_pl or _path_is_polizia(page_url):
        return True

    normalized_html = _normalize_match_text(html_text(html))
    current_root = (page_url or "").rstrip("/")
    site_root = (site_root or "").rstrip("/")
    is_homepage = bool(current_root and site_root and current_root == site_root)
    if not is_homepage:
        return False

    try:
        broad_candidates = _broad_candidate_links(html, page_url or site_root or "", limit=1, page_is_pl=False)
    except Exception:
        broad_candidates = []

    return len(normalized_html) < 400 and not broad_candidates
...
def scrape_polizia_locale(
    ...
    heavy_timeout = _phase_timeout(timeout, heavy=True)
    ...
    def _note_strong_signal(html: str, page_url: str, page_is_pl: bool) -> None:
        nonlocal strong_pl_signal_seen
        if page_is_pl or _path_is_polizia(page_url):
            strong_pl_signal_seen = True
            return
        normalized = _normalize_match_text(html)
        if any(k in normalized for k in ("polizia locale", "polizia municipale", "vigili urbani", "comando", "corpo di polizia", "servizio associato di polizia")):
            strong_pl_signal_seen = True
    ...
    try:
        ...
        for url in seed_urls:
            ...
            try:
                ...
                _harvest(text, url_is_pl=_path_is_polizia(final_url) or _path_is_polizia(url), page_url=final_url)
            except Exception:
                continue
                ...
                try:
                    ...
                    for loc in re.findall(r"<loc>(.*?)</loc>", text, flags=re.IGNORECASE | re.DOTALL):
                        ...
                        try:
                            ...
                            _harvest(text2, url_is_pl=_path_is_polizia(final_url2) or _path_is_polizia(u), page_url=final_url2)
                        except Exception:
                            continue
                except Exception:
                    continue
            ...
            for sub in ("pm", "pl", "polizia"):
                ...
                try:
                    ...
                    if status_code != 200:
                        continue
                    pages_visited.append(final_url)
                    _harvest(text, url_is_pl=True, page_url=final_url)
                    # esplora qualche link interno
                    if not (pec_all or mail_all):
                        for href, t in iter_anchors(text):
                            t = t.lower()
                            if any(k in t or k in href.lower() for k in ["contatt", "sedi"]):
        ...
        if not (pec_all or mail_all):
            try:
                ...
                _enqueue_candidate_pages(frontier, text, base, False, seen_frontier, limit=max(16, max_candidates * 6))
                ...
                while frontier and fetched_pages < 28 and time.monotonic() <= deadline and not (pec_all or mail_all):
                    ...
                    for page_url, text, is_pl in new_soup_pages:
                        if time.monotonic() > deadline or (pec_all or mail_all):
                            break
                        _enqueue_candidate_pages(frontier, text, base, is_pl, seen_frontier, min_score=2 if is_pl else 1, limit=40)
            except Exception:
                pass
        ...
        if not (pec_all or mail_all):
            try:
                ...
                if not home_html:
                    ...
                    home_html = text
                    home_url = final_url
                broad_candidates = _broad_candidate_links(home_html, base, limit=max(4, int(max_candidates) * 2), page_is_pl=True)
                if not broad_candidates and not strong_pl_signal_seen:
                    raise RuntimeError("skip broad fallback")
